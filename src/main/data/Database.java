...
public class Database {
    ...
    /**
     * <p>Creates a new Table with columns</p>
     * <p>The method will skip the creation, if the table already exists</p>
     * @param tableName only the String from a table
     * @param attributes columns of the (new) Table
     * @return a true if the Table was created, false if it already exists/error acquired
     * @throws SQLException if the sql query was faulty the Exception gets thrown
     */
    public boolean createTableIfNotExists(String tableName, String[] attributes) throws SQLException{
    ...
    /**
     * <p>Creates a new unique index for a column</p>
     * <p>The method will skip the creation, if the unique index already exists</p>
     * @param indexName name for the unique index
     * @param table name of the Table
     * @param columnName columns of the Table
     * @return a true if the Table was created, false if it already exists/error acquired
     * @throws SQLException if the sql query was faulty the Exception gets thrown
     */
    public boolean createUniqueIndexIfNotExists(String indexName, String table, String columnName) throws SQLException{
        if (executeSelectQuery("SELECT count(*) FROM sqlite_master WHERE type='index' AND name='" + indexName + "';").getInt(1) == 0) {
            String sql = "CREATE UNIQUE INDEX " + indexName + " ON " + table + " (" + columnName + ");";
            Output.write(sql, database.getClass().getSimpleName());
            startUpdateQuery(sql).addBatch();
            executeUpdateQuery();
            return true;
        }
        return false;
